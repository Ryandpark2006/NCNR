...
def objective(trial):
    ...
    smapes = smape(val[0], preds, n_jobs=1, verbose=verbose)
...
def run_trial(storage=STORAGE, cluster=None, name=None, trials=4):
    ...
    with Client(address=cluster) as client:
        ...
        futures = [
            client.submit(study.optimize, objective, n_trials=1, pure=False, callbacks = [print_callback]) for _ in range(trials)
        ]
        wait(futures)
...
if __name__ == "__main__":
    ...
    slurm_trial(name="transformer", jobs=27, trials=1000)
